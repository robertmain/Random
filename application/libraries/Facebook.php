...
class CI_Facebook {
    ...
    function __construct() {
        ...
        $this->_cache_time = $this->_obj->config->item('object_cache_time');
        $this->_caching = $this->_obj->config->item('caching');
            ...
            switch ($method) {
                case 'get':
                    if ($this->_caching) {
                        ...
                        $cacheFile = $cacheFolder . md5($uri) . '.json';
                        if (file_exists($cacheFile)) {
                            if ((time() - (filemtime($cacheFile)))/60 > $this->_cache_time) {
                                unlink($cacheFile);
                                $response = $this->connection->get($this->append_token($this->_api_url . $uri));
                                $fileHandle = fopen($cacheFile, 'w');
                                fwrite($fileHandle, json_encode($response));
                                fclose($fileHandle);
                            }
                            else {
                        ...
                        $response = $this->connection->get($this->append_token($this->_api_url . $uri));
                        $this->_obj->session->userdata('uri', $uri);
                    }

                    $response = $response->__resp->data;
                    break;
